...
func (n *Typed) PutInt64(key string, val int64) error {
	...
	return n.db.KVPut(n.prefixedKey(key), valBs[:])
}
...
func (n *Typed) Int64(key string) (int64, bool, error) {
	valBs, err := n.db.KVGet(n.prefixedKey(key))
	if err != nil {
...
func (n *Typed) PutString(key, val string) error {
	return n.db.KVPut(n.prefixedKey(key), []byte(val))
}
...
func (n *Typed) PutBytes(key string, val []byte) error {
	return n.db.KVPut(n.prefixedKey(key), val)
}
...
func (n *Typed) PutBool(key string, val bool) error {
	if val {
		return n.db.KVPut(n.prefixedKey(key), []byte{0x0})
	}
	return n.db.KVPut(n.prefixedKey(key), []byte{0x1})
}
...
func (n *Typed) Delete(key string) error {
	return n.db.KVDelete(n.prefixedKey(key))
}
