...
func newBasicAuthAndSessionMiddleware(tokenCookieManager *tokenCookieManager, loginTokens *tokenManager, guiCfg config.GUIConfiguration, ldapCfg config.LDAPConfiguration, next http.Handler, evLogger events.Logger) *basicAuthAndSessionMiddleware {
...
func (m *basicAuthAndSessionMiddleware) tryTokenLogin(w http.ResponseWriter, r *http.Request) bool {
	...
	if query.Has("token") {
		...
		// #nosec G710 - we're just deleting a query parameter from the url and not introducing anything untrusted
