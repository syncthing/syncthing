...
func newTokenManager(key string, miscDB *dbext.Typed, lifetime time.Duration, maxItems int) *tokenManager {
...
func newTokenCookieManager(shortID string, guiCfg config.GUIConfiguration, evLogger events.Logger, miscDB *dbext.Typed) *tokenCookieManager {
