...
func (s *service) Serve(ctx context.Context) error {
	...
	restMux.HandlerFunc(http.MethodGet, "/rest/cluster/unaccepted-folders", s.getUnacceptedFolders) // -
...
type unacceptedFoldersEntry struct {
	Device  protocol.DeviceID `json:"device"`
	Folders []string          `json:"folders"`
}

func (s *service) getUnacceptedFolders(w http.ResponseWriter, r *http.Request) {
	missingRemote := s.model.MissingRemoteFolders()

	entries := make([]unacceptedFoldersEntry, 0, len(missingRemote))
	for deviceID, folders := range missingRemote {
		if len(folders) > 0 {
			entries = append(entries, unacceptedFoldersEntry{deviceID, folders})
		}
	}
