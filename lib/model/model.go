...
func (m *Model) Index(deviceID protocol.DeviceID, folder string, fs []protocol.FileInfo, flags uint32, options []protocol.Option) {
	...
	fs = m.cleanIncomingIndex(folder, fs, cfg.IgnoreDelete, cfg.FollowSymlinks)
...
func (m *Model) cleanIncomingIndex(folder string, fs []protocol.FileInfo, dropDeletes, dropSymlinks bool) []protocol.FileInfo {
	for i := 0; i < len(fs); {
		switch {
		...
		case dropDeletes && fs[i].IsDeleted():
			if debug {
