...
func Test_shouldBeSwitchedOffByDefault(t *testing.T) {
	...
	if options.SingleGlobalScanner {
		t.Errorf("Expected to be disabled by default")
	}
}
...
func Test_filesystemShouldWalkInParallel(t *testing.T) {
	td := test.NewTemporaryDirectoryForTests()
	defer td.Cleanup()
	...
	model := _setUpModel(createWrapper(cfg))
...
func startFolders(ctx context.Context, cfg config.Configuration, m *Model) *testFilesystem {
...
func _setUpModel(cfg *config.Wrapper) *Model {
	db := db.OpenMemory()
	m := NewModel(cfg, protocol.LocalDeviceID, "syncthing", "dev", db, nil)
	return m
}
