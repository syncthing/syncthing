...
func TestAddFolderCompletion(t *testing.T) {
	...
	comp := newFolderCompletion(db.Counts{}, db.Counts{}, 0, events.RemoteFolderValid)
	comp.add(newFolderCompletion(db.Counts{}, db.Counts{}, 0, events.RemoteFolderPaused))
	if comp.CompletionPct != 100 {
		t.Error(comp.CompletionPct)
	}

	// Completion is of the whole
	comp = newFolderCompletion(db.Counts{Bytes: 100}, db.Counts{}, 0, events.RemoteFolderValid)             // 100% complete
	comp.add(newFolderCompletion(db.Counts{Bytes: 400}, db.Counts{Bytes: 50}, 0, events.RemoteFolderValid)) // 82.5% complete
