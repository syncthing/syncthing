...
type folder struct {
	...
	*stats.FolderStatisticsReference
	...
	scanErrors []events.FileError
	pullErrors []events.FileError
...
func (f *folder) monitorWatch(ctx context.Context) {
	...
	for {
		...
		case err = <-errChan:
			...
			aggrCtx, aggrCancel = context.WithCancel(ctx)
		case ev := <-summaryChan:
			if data, ok := ev.Data.(events.FolderSummaryEventData); !ok {
...
func (f *folder) setWatchError(err error, nextTryIn time.Duration) {
	...
	f.watchMut.Unlock()
	if err != prevErr {
		data := events.FolderWatchStateChangedEventData{
			Folder: f.ID,
		}
		if prevErr != nil {
			tmp := prevErr.Error()
...
func (f *folder) newScanError(path string, err error) {
	...
	f.scanErrors = append(f.scanErrors, events.FileError{
		Err:  err.Error(),
		Path: path,
	})
	f.errorsMut.Unlock()
...
func (f *folder) Errors() []events.FileError {
	f.errorsMut.Lock()
	defer f.errorsMut.Unlock()
	scanLen := len(f.scanErrors)
	errors := make([]events.FileError, scanLen+len(f.pullErrors))
...
func (f *folder) updateLocals(fs []protocol.FileInfo) {
	...
	f.evLogger.Log(events.LocalIndexUpdated, events.LocalIndexUpdatedEventData{
		Folder:    f.ID,
		Items:     len(fs),
		Filenames: filenames,
		Sequence:  seq,
...
func (f *folder) emitDiskChangeEvents(fs []protocol.FileInfo, typeOfEvent events.EventType) {
	for _, file := range fs {
		...
		f.evLogger.Log(typeOfEvent, events.DiskChangeDetectedEventData{
			Folder:     f.ID,
