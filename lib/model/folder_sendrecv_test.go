...
func createEmptyFileInfo(t *testing.T, name string, fs fs.Filesystem) protocol.FileInfo {
	...
	must(t, err)
	file, err := scanner.CreateFileInfo(fi, name, fs, false, false, config.XattrFilter{}, false)
	must(t, err)
	return file
}
...
func TestDeleteIgnorePerms(t *testing.T) {
	...
	stat, err := file.Stat()
	must(t, err)
	fi, err := scanner.CreateFileInfo(stat, name, ffs, false, false, config.XattrFilter{}, false)
	must(t, err)
	ffs.Chmod(name, 0600)
