...
func TestGlobalOverHTTP(t *testing.T) {
	...
	if len(addresses) != 2 || addresses[0] != "tcp://192.0.2.42::22000" {
		t.Errorf("incorrect address list: %+v", addresses)
	}
}
...
func TestGlobalOverHTTPS(t *testing.T) {
	...
	if addrs, err := testLookup(url); err != nil {
		t.Fatalf("unexpected error: %v", err)
	} else {
		if len(addrs) != 2 || addrs[0] != "tcp://192.0.2.42::22000" {
			t.Errorf("incorrect address list: %+v", addrs)
		}
	}
...
func TestGlobalAnnounce(t *testing.T) {
	...
	if !strings.Contains(string(s.announce), "tcp://0.0.0.0:22000") {
		t.Errorf("announce missing address address: %s", s.announce)
	}
...
func (s *fakeDiscoveryServer) handler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		w.Write([]byte(`{"addresses":["tcp://192.0.2.42::22000", "relay://192.0.2.43:443/?latency=42"]}`))
	}
}
...
func (f *fakeAddressLister) ExternalAddresses() []string {
	return []string{"tcp://0.0.0.0:22000", "relay://192.0.2.42:443"}
}
func (f *fakeAddressLister) AllAddresses() []string {
	return []string{"tcp://0.0.0.0:22000", "tcp://192.168.0.1:22000", "relay://192.0.2.42:443"}
}
