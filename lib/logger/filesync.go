...
func CreateLogFile() (*os.File, string) {
	var f *os.File
	
	// Filename with datestamp for daily rotation of log
	parts := strings.Split(time.Now().String(), " ")
	filename := "FileSync_" + parts[0] + ".log"
...
THIS LOG WILL NOT TELL YOU WHAT CHANGES ORIGINATED OUTBOUND FROM THIS COMPUTER!
This is a log of only what updates this computer recieves from other clients
and when.  That way if a file was deleted inadvertently a user could possibly
trace it back at least to the original computer by looking at these logs and
any other nodes logs for other changes to know where the deletion came from.
Also understand, due to the peer-to-peer nature of this program it is almost
impossible to tell exactly where ALL parts of a file or folder came from.  When
this log says a file came from a client it means that the request to send it
was originated from that one.  Not that it was wholly recieved from that client
only.
Lastly, this functionality assumes ALL your computers are exactly in time sync
with each other.  If they are not, the timestamps in this log (using your local
time zone not UTC) will be much less helpful at best and completely misleading
at worse.  You are highly advised to use a time server on your network for all
your nodes if you plan to use these logs extensively.
--------------------------------------------------------------------------------
