...
func (w *Walker) Walk() (chan protocol.FileInfo, error) {
	...
	go func() {
		hashFiles := w.walkAndHashFiles(files, hashedFiles, &extraDirs)
		if len(w.Subs) == 0 {
...
func (w *Walker) walkAndHashFiles(fchan, dchan chan protocol.FileInfo, extraDirs *[]string) filepath.WalkFunc {
	now := time.Now()
