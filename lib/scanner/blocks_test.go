...
func TestBlocks(t *testing.T) {
	for _, test := range blocksTestData {
		...
		blocks, err := Blocks(SHA256, buf, test.blocksize, 0, nil)
...
func TestDiff(t *testing.T) {
	for i, test := range diffTestData {
		a, _ := Blocks(Murmur3, bytes.NewBufferString(test.a), test.s, 0, nil)
		b, _ := Blocks(Murmur3, bytes.NewBufferString(test.b), test.s, 0, nil)
		_, d := BlockDiff(a, b)
		if len(d) != len(test.d) {
