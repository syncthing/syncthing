...
func (f FileInfo) IsEquivalent(other FileInfo, modTimeWindowS int) bool {
	return f.isEquivalent(other, modTimeWindowS, false, false, 0)
}

func (f FileInfo) IsEquivalentOptional(other FileInfo, modTimeWindowS int, ignorePerms bool, ignoreBlocks bool, ignoreFlags uint32) bool {
	return f.isEquivalent(other, modTimeWindowS, ignorePerms, ignoreBlocks, ignoreFlags)
}
...
//  - modification time (difference bigger than modTimeWindowS)
//  - size
...
func (f FileInfo) isEquivalent(other FileInfo, modTimeWindowS int, ignorePerms bool, ignoreBlocks bool, ignoreFlags uint32) bool {
	...
	switch f.Type {
	case FileInfoTypeFile:
		return f.Size == other.Size && ModTimeEqual(f.ModTime(), other.ModTime(), modTimeWindowS) && (ignoreBlocks || BlocksEqual(f.Blocks, other.Blocks))
...
func ModTimeEqual(a, b time.Time, modTimeWindowS int) bool {
	if a == b {
		return true
	}
	diff := a.Sub(b)
	...
	return diff < time.Duration(modTimeWindowS)*time.Second
}
