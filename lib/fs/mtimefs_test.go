...
func (s mapStore) PutMtimePair(_, name string, real, virtual time.Time) error {
	s[name] = [2]time.Time{real, virtual}
	return nil
}

func (s mapStore) GetMtimePair(_, name string) (real, virtual time.Time) {
	v := s[name]
	return v[0], v[1]
}

func (s mapStore) DeleteMtimePair(_, name string) error {
	delete(s, name)
	return nil
}
