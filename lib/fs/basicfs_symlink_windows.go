...
func (BasicFilesystem) CreateSymlink(source, LinkTarget string, tt LinkTargetType) error {
	...
	trgp, err := syscall.UTF16PtrFromString(osutil.NativeFilename(LinkTarget))
	if err != nil {
		return err
	}
	...
	// If the flags doesn't reveal the LinkTarget type, try to evaluate it
	// ourselves, and worst case default to the symlink pointing to a file.
	mode := 0
	if tt == LinkTargetUnknown {
		path := LinkTarget
		if !filepath.IsAbs(LinkTarget) {
			path = filepath.Join(filepath.Dir(source), LinkTarget)
		}
