...
func (t *tcpListener) serve(ctx context.Context) error {
	...
	if err != nil {
		slog.Warn("Failed to listen (TCP)", slogutil.Error(err))
		return err
	}
	...
	slog.Info("TCP listener starting", slogutil.Address(tcaddr))
	defer slog.Info("TCP listener shutting down", slogutil.Address(tcaddr))
	...
	for {
		...
		if err := tlsTimedHandshake(tc); err != nil {
			slog.Warn("Failed TLS handshake", slogutil.Address(tc.RemoteAddr()), slogutil.Error(err))
			tc.Close()
			continue
		}
