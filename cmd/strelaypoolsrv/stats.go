...
func init() {
	prometheus.MustRegister(prometheus.NewProcessCollector(os.Getpid(), "syncthing_relaypool"))
}
...
var (
	...
	relayUptime             = makeGauge("relay_uptime", "Uptime of relay", "relaypoolsrv")
	relayPendingSessionKeys = makeGauge("relay_pending_session_keys", "Number of pending session keys (two keys per session, one per each side of the connection)", "relaypoolsrv")
	relayActiveSessions     = makeGauge("relay_active_sessions", "Number of sessions that are happening, a session contains two parties", "relaypoolsrv")
	relayConnections        = makeGauge("relay_connections", "Number of devices connected to the relay", "relaypoolsrv")
	relayProxies            = makeGauge("relay_proxies", "Number of active proxy routines sending data between peers (two proxies per session, one for each way)", "relaypoolsrv")
	relayBytesProxied       = makeGauge("relay_bytes_proxied", "Number of bytes proxied by the relay", "relaypoolsrv")
	relayGoRoutines         = makeGauge("relay_go_routines", "Number of Go routines in the process", "relaypoolsrv")
	relaySessionRate        = makeGauge("relay_session_rate", "Rate applied per session", "relaypoolsrv")
	relayGlobalRate         = makeGauge("relay_global_rate", "Global rate applied on the whole relay", "relaypoolsrv")
	relayBuildInfo          = makeGauge("relay_build_info", "Build information about a relay", "relaypoolsrv", "go_version", "go_os", "go_arch")
	relayLocationInfo       = makeGauge("relay_location_info", "Location information about a relay", "relaypoolsrv", "city", "country", "continent")
)
