...
func cpuBench(iterations int, duration time.Duration) float64 {
	...
	for i := 0; i < iterations; i++ {
		if v := cpuBenchOnce(duration, bs); v > perf {
			perf = v
		}
	}
	blocksResult = nil
...
func cpuBenchOnce(duration time.Duration, bs []byte) float64 {
	t0 := time.Now()
	b := 0
	for time.Since(t0) < duration {
		r := bytes.NewReader(bs)
		blocksResult, _ = scanner.Blocks(r, protocol.BlockSize, int64(len(bs)), nil, true)
		b += len(bs)
	}
	d := time.Since(t0)
