...
/*jslint browser: true, continue: true, plusplus: true, devel: true, nomen: true, stupid: true, bitwise: true */
/*global $: false, angular: false, validLangs: false */
(function () {
    'use strict';
    ...
    syncthing.controller('SyncthingCtrl', function ($scope, $http, $translate, $location) {
            ...
            if (!debouncedFuncs[key]) {
                debouncedFuncs[key] = debounce(function () {
                    $http.get(urlbase + '/completion?node=' + node + '&repo=' + encodeURIComponent(repo)).success(function (data) {
                        ...
                        var tot = 0, cnt = 0, cmp;
                        for (cmp in $scope.completion[node]) {
                            if (cmp === "_total") {
                                continue;
                            }
        ...
        $scope.editRepo = function (nodeCfg) {
            ...
            // actually undefined.
            if ($scope.currentRepo.staggeredMaxAge === undefined) {
        ...
        $scope.saveRepo = function () {
            ...
            for (nodeID in repoCfg.selectedNodes) {
        ...
        $scope.needAction = function (file) {
            ...
            if ((file.Flags & fDelete) === fDelete) {
                return 'rm';
            }
            if ((file.Flags & fDirectory) === fDirectory) {
                return 'touch';
            }
            return 'sync';
        };
