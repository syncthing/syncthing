angular
  ...
  .controller("SyncthingController", function(
    ...
    $scope.$on(Events.ONLINE, function() {
      ...
      $http
        .get(urlbase + "/system/version")
        .success(function(data) {
          console.log("version", data);
          if (
            $scope.version.version &&
            $scope.version.version !== data.version
          ) {
    ...
    $scope.$on("ConfigLoaded", function() {
      if ($scope.config.options.urAccepted === 0) {
        ...
        var firstVisit = document.cookie.replace(
          /(?:(?:^|.*;\s*)firstVisit\s*\=\s*([^;]*).*$)|^.*$/,
          "$1"
        );
        if (!firstVisit) {
          document.cookie =
            "firstVisit=" + Date.now() + ";max-age=" + 30 * 24 * 3600;
        } else {
    ...
    $scope.$on(Events.CONFIG_SAVED, function(event, arg) {
      ...
      $http
        .get(urlbase + "/system/config/insync")
        .success(function(data) {
    ...
    $scope.$on(Events.DOWNLOAD_PROGRESS, function(event, arg) {
      ...
      for (var folder in stats) {
        ...
        for (var file in stats[folder]) {
          ...
          if (
            pulling < 1 &&
            pulled + copiedFromElsewhere + copiedFromOrigin + reused <= 99
          ) {
            pulling = 1;
          }
    ...
    function refreshFolder(folder) {
      ...
      if (!debouncedFuncs[key]) {
        debouncedFuncs[key] = debounce(function() {
          $http
            .get(urlbase + "/db/status?folder=" + encodeURIComponent(folder))
            .success(function(data) {
    ...
    function updateLocalConfig(config) {
      ...
      $scope.config.options._urAcceptedStr =
        "" + $scope.config.options.urAccepted;
        ...
        .success(function(data) {
          ...
          for (var address in data.connectionServiceStatus) {
            if (data.connectionServiceStatus[address].error) {
              listenersFailed.push(
                address + ": " + data.connectionServiceStatus[address].error
              );
            }
          }
    ...
    function recalcCompletion(device) {
      ...
      if (total == 0) {
        ...
        $scope.completion[device]._total = Math.floor(
          100 * (1 - needed / total)
        );
    ...
    function refreshConnectionStats() {
      $http
        .get(urlbase + "/system/connections")
        .success(function(data) {
          var now = Date.now(),
          ...
          try {
            data.total.inbps = Math.max(
              0,
              (data.total.inBytesTotal - $scope.connectionsTotal.inBytesTotal) /
                td
            );
            data.total.outbps = Math.max(
              0,
              (data.total.outBytesTotal -
                $scope.connectionsTotal.outBytesTotal) /
                td
            );
          } catch (e) {
            data.total.inbps = 0;
          ...
          for (id in data) {
            ...
            try {
              data[id].inbps = Math.max(
                0,
                (data[id].inBytesTotal - $scope.connections[id].inBytesTotal) /
                  td
              );
              data[id].outbps = Math.max(
                0,
                (data[id].outBytesTotal -
                  $scope.connections[id].outBytesTotal) /
                  td
              );
            } catch (e) {
              data[id].inbps = 0;
    ...
    $scope.refreshNeed = function(page, perpage) {
      ...
      var url =
        urlbase + "/db/need?folder=" + encodeURIComponent($scope.neededFolder);
      url += "&page=" + page;
      url += "&perpage=" + perpage;
      $http
        .get(url)
    ...
    function shouldSetDefaultFolderPath() {
      return (
        ...
        $scope.config.options.defaultFolderPath &&
        !$scope.editingExisting &&
        $scope.folderEditor.folderPath.$pristine
      );
    }
    ...
    function saveIgnores(ignores, cb) {
      $http
        .post(
          urlbase +
            "/db/ignores?folder=" +
            encodeURIComponent($scope.currentFolder.id),
          {
            ignore: ignores
          }
        )
    ...
    $scope.refreshFailed = function(page, perpage) {
      ...
      var url =
        ...
        encodeURIComponent($scope.failed.folder);
    ...
    $scope.refreshRemoteNeed = function(folder, page, perpage) {
      ...
      var url =
        urlbase + "/db/remoteneed?device=" + $scope.remoteNeedDevice.deviceID;
    ...
    var refreshDeviceStats = debounce(function() {
      $http
        .get(urlbase + "/stats/device")
        .success(function(data) {
          $scope.deviceStats = data;
          for (var device in $scope.deviceStats) {
            $scope.deviceStats[device].lastSeen = new Date(
              $scope.deviceStats[device].lastSeen
            );
            $scope.deviceStats[device].lastSeenDays =
              (new Date() - $scope.deviceStats[device].lastSeen) / 1000 / 86400;
          }
    ...
    var refreshFolderStats = debounce(function() {
      $http
        .get(urlbase + "/stats/folder")
        .success(function(data) {
          $scope.folderStats = data;
          for (var folder in $scope.folderStats) {
            if ($scope.folderStats[folder].lastFile) {
              $scope.folderStats[folder].lastFile.at = new Date(
                $scope.folderStats[folder].lastFile.at
              );
            }

            $scope.folderStats[folder].lastScan = new Date(
              $scope.folderStats[folder].lastScan
            );
            $scope.folderStats[folder].lastScanDays =
              (new Date() - $scope.folderStats[folder].lastScan) / 1000 / 86400;
          }
    ...
    var refreshThemes = debounce(function() {
      $http
        .get("themes.json")
        .success(function(data) {
          // no urlbase here as this is served by the asset handler
    ...
    var refreshGlobalChanges = debounce(function() {
      $http
        .get(urlbase + "/events/disk?limit=25")
        .success(function(data) {
          if (!data) {
    ...
    $scope.syncPercentage = function(folder) {
      ...
      if (
        $scope.model[folder].needBytes == 0 &&
        $scope.model[folder].needDeletes > 0
      ) {
        // We don't need any data, but we have deletes that we need
        // to do. Drop down the completion percentage to indicate
        // that we have stuff to do.
      ...
      var pct =
        (100 * $scope.model[folder].inSyncBytes) /
        $scope.model[folder].globalBytes;
    ...
    $scope.scanPercentage = function(folder) {
      ...
      var pct =
        (100 * $scope.scanProgress[folder].current) /
        $scope.scanProgress[folder].total;
      return Math.floor(pct);
    };
    ...
    $scope.scanRemaining = function(folder) {
      ...
      var remainingBytes =
        $scope.scanProgress[folder].total - $scope.scanProgress[folder].current;
    ...
    $scope.syncthingStatus = function() {
      ...
      if (
        ...
        !online ||
        (!isEmptyObject($scope.config) &&
          ($scope.config.pendingDevices.length > 0 || pendingFolders > 0))
      ) {
        notifyCount++;
      }
    ...
    $scope.deviceName = function(deviceCfg) {
      if (
        typeof deviceCfg === "undefined" ||
        typeof deviceCfg.deviceID === "undefined"
      ) {
        return "";
      }
      if (deviceCfg.name) {
    ...
    $scope.logging = {
      ...
      onScroll: function() {
        ...
        $scope.logging.paused =
          scrollHeight > scrollTop + textArea.outerHeight();
      ...
      fetch: function() {
        ...
        $http
          ...
          .success(function(data) {
            ...
            if (!$scope.logging.paused) {
              if (data.messages) {
                $scope.logging.entries.push.apply(
                  $scope.logging.entries,
                  data.messages
                );
    ...
    $scope.showSettings = function() {
      ...
      $scope.tmpRemoteIgnoredDevices = angular.copy(
        $scope.config.remoteIgnoredDevices
      );
    ...
    $scope.saveConfig = function(callback) {
      ...
      $http
        .post(urlbase + "/system/config", cfg, opts)
        .success(function() {
          refreshConfig();
    ...
    $scope.settingsModified = function() {
      ...
      var ignoredDevicesEquals = angular.equals(
        $scope.config.remoteIgnoredDevices,
        $scope.tmpRemoteIgnoredDevices
      );
      var ignoredFoldersEquals = angular.equals(
        $scope.config.devices,
        $scope.tmpDevices
      );
      console.log(
        "settings equals - options: " +
          optionsEqual +
          " gui: " +
          guiEquals +
          " ignDev: " +
          ignoredDevicesEquals +
          " ignFol: " +
          ignoredFoldersEquals
      );
      return (
        !optionsEqual ||
        !guiEquals ||
    ...
    $scope.saveSettings = function() {
      ...
      if ($scope.settingsModified()) {
        ...
        $scope.tmpOptions.urAccepted = parseInt(
          $scope.tmpOptions._urAcceptedStr
        );
        ...
        if ($scope.tmpOptions.upgrades == "candidate") {
          $scope.tmpOptions.autoUpgradeIntervalH =
            $scope.tmpOptions.autoUpgradeIntervalH || 12;
        ...
        ["listenAddresses", "globalAnnounceServers"].forEach(function(key) {
          $scope.tmpOptions[key] = $scope.tmpOptions["_" + key + "Str"]
            .split(/[ ,]+/)
            .map(function(x) {
              return x.trim();
            });
        ...
        $scope.thisDeviceIn($scope.tmpDevices).name =
          $scope.tmpOptions.deviceName;
        $scope.config.options = angular.copy($scope.tmpOptions);
        $scope.config.gui = angular.copy($scope.tmpGUI);
        $scope.config.remoteIgnoredDevices = angular.copy(
          $scope.tmpRemoteIgnoredDevices
        );
    ...
    $scope.deleteDevice = function() {
      ...
      for (var id in $scope.folders) {
        $scope.folders[id].devices = $scope.folders[id].devices.filter(function(
          n
        ) {
          return n.deviceID !== $scope.currentDevice.deviceID;
        });
    ...
    $scope.saveDeviceConfig = function(deviceCfg) {
      ...
      for (var id in deviceCfg.selectedFolders) {
        if (deviceCfg.selectedFolders[id]) {
          ...
          $scope.folders[id].devices = $scope.folders[id].devices.filter(
            function(n) {
    ...
    $scope.unignoreDeviceFromTemporaryConfig = function(ignoredDevice) {
      $scope.tmpRemoteIgnoredDevices = $scope.tmpRemoteIgnoredDevices.filter(
        function(existingIgnoredDevice) {
    ...
    $scope.unignoreFolderFromTemporaryConfig = function(
      device,
      ignoredFolderID
    ) {
      for (var i = 0; i < $scope.tmpDevices.length; i++) {
        if ($scope.tmpDevices[i].deviceID == device) {
          $scope.tmpDevices[i].ignoredFolders = $scope.tmpDevices[
            i
          ].ignoredFolders.filter(function(existingIgnoredFolder) {
    ...
    $scope.fsWatcherErrorMap = function() {
      ...
      $.each($scope.folders, function(id, cfg) {
        if (
          cfg.fsWatcherEnabled &&
          $scope.model[cfg.id] &&
          $scope.model[id].watchError &&
          !cfg.paused &&
          $scope.folderStatus(cfg) !== "stopped"
        ) {
    ...
    $scope.$watch("currentFolder.label", function(newvalue) {
      ...
      $scope.currentFolder.path = pathJoin(
        $scope.config.options.defaultFolderPath,
        newvalue
      );
    });
    ...
    $scope.$watch("currentFolder.id", function(newvalue) {
      if (
        !newvalue ||
        !shouldSetDefaultFolderPath() ||
        $scope.currentFolder.label
      ) {
        return;
      }
      $scope.currentFolder.path = pathJoin(
    ...
    $scope.editFolder = function(folderCfg) {
      ...
      $scope.currentFolder.unrelatedDevices = $scope.devices.filter(function(
        n
      ) {
        return (
      ...
      if (
        $scope.currentFolder.versioning &&
        $scope.currentFolder.versioning.type === "trashcan"
      ) {
        $scope.currentFolder.trashcanFileVersioning = true;
        $scope.currentFolder.fileVersioningSelector = "trashcan";
        $scope.currentFolder.trashcanClean = +$scope.currentFolder.versioning
          .params.cleanoutDays;
        $scope.currentFolder.versioningCleanupIntervalS = +$scope.currentFolder
          .versioning.cleanupIntervalS;
      } else if (
        $scope.currentFolder.versioning &&
        $scope.currentFolder.versioning.type === "simple"
      ) {
        $scope.currentFolder.simpleFileVersioning = true;
        $scope.currentFolder.fileVersioningSelector = "simple";
        $scope.currentFolder.simpleKeep = +$scope.currentFolder.versioning
          .params.keep;
        $scope.currentFolder.trashcanClean = +$scope.currentFolder.versioning
          .params.cleanoutDays;
        $scope.currentFolder.versioningCleanupIntervalS = +$scope.currentFolder
        ...
        $scope.currentFolder.versioning &&
        $scope.currentFolder.versioning.type === "staggered"
      ) {
        $scope.currentFolder.staggeredFileVersioning = true;
        $scope.currentFolder.fileVersioningSelector = "staggered";
        $scope.currentFolder.staggeredMaxAge = Math.floor(
          +$scope.currentFolder.versioning.params.maxAge / 86400
        );
        $scope.currentFolder.staggeredCleanInterval = +$scope.currentFolder
          .versioning.params.cleanInterval;
        $scope.currentFolder.staggeredVersionsPath =
          $scope.currentFolder.versioning.params.versionsPath;
        $scope.currentFolder.versioningCleanupIntervalS = +$scope.currentFolder
          .versioning.cleanupIntervalS;
      } else if (
        $scope.currentFolder.versioning &&
        $scope.currentFolder.versioning.type === "external"
      ) {
        ...
        $scope.currentFolder.externalCommand =
          $scope.currentFolder.versioning.params.command;
      } else {
        $scope.currentFolder.fileVersioningSelector = "none";
      }
      $scope.currentFolder.trashcanClean =
        $scope.currentFolder.trashcanClean || 0; // weeds out nulls and undefineds
      $scope.currentFolder.simpleKeep = $scope.currentFolder.simpleKeep || 5;
      $scope.currentFolder.staggeredCleanInterval =
        $scope.currentFolder.staggeredCleanInterval || 3600;
      $scope.currentFolder.staggeredVersionsPath =
        $scope.currentFolder.staggeredVersionsPath || "";
      $scope.currentFolder.versioningCleanupIntervalS =
        $scope.currentFolder.versioningCleanupIntervalS || 0;
      ...
      $http
        ...
        .error(function(err) {
          $scope.ignores.text = $translate.instant(
    ...
    $scope.addFolder = function() {
      $http
        .get(urlbase + "/svc/random/string?length=10")
        .success(function(data) {
          $scope.editingExisting = false;
          $scope.currentFolder = angular.copy($scope.folderDefaults);
          $scope.currentFolder.id = (
            data.random.substr(0, 5) +
            "-" +
            data.random.substr(5, 5)
          ).toLowerCase();
      ...
      $scope.restoreVersions = {
        ...
        massAction: function(name, action) {
          $.each($scope.restoreVersions.versions, function(key) {
            if (
              key.startsWith(name + "/") &&
              (!$scope.restoreVersions.filters.text ||
                key.indexOf($scope.restoreVersions.filters.text) > -1)
            ) {
              if (action == "unset") {
              ...
              $.each(
                $scope.restoreVersions.filterVersions(
                  $scope.restoreVersions.versions[key]
                ),
        ...
        filterVersions: function(versions) {
          ...
          $.each(versions, function(idx, version) {
            if (
              moment(version.versionTime).isBetween(
                $scope.restoreVersions.filters["start"],
                $scope.restoreVersions.filters["end"],
                null,
                "[]"
              )
            ) {
          ...
          $http
            .post(
              urlbase +
                ...
                encodeURIComponent($scope.restoreVersions.folder),
              selections
            )
            .success(function(data) {
        ...
        show: function(folder) {
          ...
          $q.all([dataReceived, modalShown.promise]).then(function() {
            $timeout(function() {
              ...
              $scope.restoreVersions.tree = $("#restoreTree")
                .fancytree({
                  ...
                  renderColumns: function(event, data) {
                    ...
                    if (node.folder) {
                      template =
                        '<div ng-include="\'syncthing/folder/restoreVersionsMassActions.html\'" class="pull-right"/>';
                    } else {
                      template =
                        '<div ng-include="\'syncthing/folder/restoreVersionsVersionSelector.html\'" class="pull-right"/>';
                    }
                    ...
                    $tdList.eq(1).html($compile(template)(scope));
              ...
              // Find version window.
              $.each($scope.restoreVersions.versions, function(key) {
                $.each($scope.restoreVersions.versions[key], function(
                  idx,
                  version
                ) {
              ...
              $.each(ranges, function(key, range) {
                if (
                  !range[0].isBetween(minDate, maxDate, null, "[]") &&
                  !range[1].isBetween(minDate, maxDate, null, "[]")
                ) {
                  delete ranges[key];
                }
              });
    ...
    $scope.$watchCollection("restoreVersions.filters", function() {
      ...
      $scope.restoreVersions.tree.filterNodes(function(node) {
        ...
        if (
          $scope.restoreVersions.filters.text &&
          node.key.indexOf($scope.restoreVersions.filters.text) < 0
        ) {
          return false;
        }
        if (
          $scope.restoreVersions.filterVersions(node.data.versions).length == 0
        ) {
          return false;
        }
        return true;
    ...
    $scope.setAPIKey = function(cfg) {
      $http
        .get(urlbase + "/svc/random/string?length=32")
        .success(function(data) {
    ...
    $scope.refreshReportDataPreview = function(ver, diff) {
      ...
      if (diff && version > 2) {
        ...
        $http
          .get(urlbase + "/svc/report?version=" + version)
          .success(function(data) {
